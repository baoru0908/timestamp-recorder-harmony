...
| 版本 | 内容 |
|---|---|
| **v1.4.4** | **卡片候选精简 + 跨卡同步修复**：候选列表移除与「单事件 1×2」完全重复的胶囊卡（9 → 8 个条目），顺序重排为「全部事件 → 单事件 → 自定义」（存量胶囊卡仍可正常使用）；修复「在一张卡片上记录后，其他桌面卡片不刷新」（v1.4.2 缩短刷新链路时的回归），点击后先一步刷新被点卡、再补刷其余卡，并对连点做合并防重复刷新 |
