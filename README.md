...
| 版本 | 内容 |
|---|---|
| **v1.4.5** | **隐私声明与华为托管页对齐**：应用内隐私声明改为与华为「标准化隐私声明托管服务」（AGC 提审所填链接）**同构**——收集使用 / 权利行使 / 存储地点及期限 / 联系我们 / 第三方共享与 SDK / 未成年人保护 / 政策更新；采纳托管页关键表述「数据仅存储在设备本地应用私有空间，不通过网络传输」「保存期限为实现处理目的所必要的最短时间」。**补足托管页缺失的三项**：权利行使途径（设置 → 隐私 → 撤回隐私同意等）、第三方共享与 SDK 声明（无）、未成年人保护；**修正托管页两处易生歧义的措辞**：「需要收集您的事件操作记录」→「不收集您的任何个人信息」、「传输并保存至您本地的服务器」→「仅保存在您的设备本地，不会传输至任何服务器」 |
